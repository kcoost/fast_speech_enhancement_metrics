...
def load_noise(num_samples: int = 1, sample_duration: int = 1, sample_rate: int = 16000):
    ...
    noise_list = []
    total_samples = 0
    progress_bar = tqdm(flap_noise["train"], desc="Loading noise", mininterval=5)
    for sample in progress_bar:
        noise = resample(
            torch.tensor(sample["audio"]["array"]), orig_freq=sample["audio"]["sampling_rate"], new_freq=sample_rate
        )
        ...
        progress_bar.set_description(
            f"Loading noise: {100*total_samples/(total_duration * sample_rate):.2f}%", refresh=False
        )
...
def load_speech(num_samples: int = 1, sample_duration: int = 1, sample_rate: int = 16000):
    ...
    speech_list = []
    total_samples = 0
    progress_bar = tqdm(peoples_speech["validation"], desc="Loading speech", mininterval=5)
    for sample in progress_bar:
        speech = resample(
            torch.tensor(sample["audio"]["array"]), orig_freq=sample["audio"]["sampling_rate"], new_freq=sample_rate
        )
        ...
        if total_samples >= total_duration * sample_rate:
            break
        progress_bar.set_description(
            f"Loading speech: {100*total_samples/(total_duration * sample_rate):.2f}%", refresh=False
        )
...
def load_audio_data(
    sample_duration: int = 1, num_samples: int = 1, sample_rate: int = 16000, SNR_high: int = 25, SNR_low: int = -5
):
