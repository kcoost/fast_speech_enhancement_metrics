...
class Pesq:
    ...
    def __init__(
        ...
        self.filter_bank = BarkFilterBank(n_fft // 2, nbarks)
    ...
    def equalize_bark_bands(self, clean_bark_bands: torch.Tensor, noisy_bark_bands: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        band_power_ratio = (mean_clean_band_power + 1000) / (mean_noisy_band_power + 1000)
    ...
    def get_disturbances(self, clean_speech: torch.Tensor, noisy_speech: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        symm_distu = self.filter_bank.weighted_norm(disturbance, p=2)
        symm_distu = symm_distu.clamp(min=1e-20)

        # asymmetrical disturbance
        asymm_scaling = ((equalized_noisy_bark_bands + 50.0) / (equalized_clean_bark_bands + 50.0)) ** 1.2
        asymm_scaling[asymm_scaling < 3.0] = 0.0
        asymm_scaling = asymm_scaling.clamp(max=12.0)

        asymm_distu = self.filter_bank.weighted_norm(disturbance * asymm_scaling, p=1)
        asymm_distu = asymm_distu.clamp(min=1e-20)

        # weighting
        h = ((self.loudness.total_audible(equalized_clean_bark_bands, 1) + 1e5) / 1e7) ** 0.04
    ...
    def __call__(self, clean_speech: torch.Tensor, noisy_speech: torch.Tensor) -> torch.Tensor:
        d_symm, d_asymm = self.get_disturbances(clean_speech, noisy_speech)
